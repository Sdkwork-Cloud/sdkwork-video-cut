...
function installersForPlatform(platform) {
  ...
  if (platform === 'windows-x86_64') {
    return [
      { kind: 'msi', path: 'release/SDKWork.Video.Cut_0.1.4_x64_en-US.msi', byteSize: 10, sha256: sha256[0] },
      { kind: 'nsis', path: 'release/SDKWork.Video.Cut_0.1.4_x64-setup.exe', byteSize: 11, sha256: sha256[1] },
    ];
  }
  if (platform === 'linux-x86_64') {
    return [
      { kind: 'deb', path: 'release/SDKWork.Video.Cut_0.1.4_amd64.deb', byteSize: 12, sha256: sha256[0] },
      { kind: 'appimage', path: 'release/SDKWork.Video.Cut_0.1.4_amd64.AppImage', byteSize: 13, sha256: sha256[1] },
    ];
  }
  return [
    { kind: 'dmg', path: `release/SDKWork.Video.Cut_0.1.4_${platform}.dmg`, byteSize: 14, sha256: sha256[0] },
    { kind: 'app', path: `release/SDKWork.Video.Cut_0.1.4_${platform}.app.tar.gz`, byteSize: 15, sha256: sha256[1] },
  ];
}
...
function writeSbomEvidence(root, overrides = {}) {
  const packages = [
    ...
    url: `https://github.com/Sdkwork-Cloud/sdkwork-video-cut/releases/download/v0.1.4/${fileName}`,
    byteSize: 100,
    sha256,
  }));
...
assert.equal(readyReport.releaseTag, 'v0.1.4');
...
assert.equal(
  ...
  'ok - autocut release evidence status domains=7/7 blockers=0 releaseTag=v0.1.4',
);
...
assert.ok(
  blockedReport.nextActions.some(
    (action) =>
      ...
      action.command.includes('pnpm release:environment -- --release-tag v0.1.4 --json'),
  ),
);
assert.ok(
  blockedReport.nextActions.some(
...
assert.equal(
  ...
  `blocked - autocut release evidence status domains=1/7 blockers=${blockedReport.summary.blockerCount} releaseTag=v0.1.4`,
);
