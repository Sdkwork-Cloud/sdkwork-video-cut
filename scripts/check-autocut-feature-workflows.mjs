...
assertIncludes(slicerPageForDedup, 'Smart dedup review', 'Smart Slice review workbench surfaces service-side video dedup findings');
...
assertIncludes(slicerPage, 'SMART_CUT_ENGINE_PIPELINE_STEPS', 'Slicer UI defines the commercial Smart Cut Engine pipeline steps as a first-class product surface');
...
assertIncludes(slicerPage, 'createSmartCutEngineProductExperience', 'Slicer UI derives one product experience model from the selected mode and publishing settings');
...
assertIncludes(slicerPage, 'Scene Strategy', 'Slicer UI surfaces the selected engine scene strategy as an operator decision card');
assertIncludes(slicerPage, 'Evidence Gate', 'Slicer UI surfaces transcript and speaker evidence gates before execution');
assertIncludes(slicerPage, 'Boundary Contract', 'Slicer UI surfaces semantic boundary policy as a product contract');
assertIncludes(slicerPage, 'Review Contract', 'Slicer UI surfaces candidate-id review as a product contract');
assertIncludes(slicerPage, 'Output Package', 'Slicer UI surfaces commercial deliverables generated by the Smart Cut Engine');
assertIncludes(slicerPage, 'Operator Brief', 'Slicer UI presents the selected smart-edit job as an operator brief instead of raw algorithm knobs');
assertIncludes(slicerPage, 'Quality Gate', 'Slicer UI surfaces publishing quality gates before execution');
assertIncludes(slicerPage, 'Fail Closed', 'Slicer UI tells operators that weak transcript or speaker evidence blocks commercial slicing');
assertRule(
  !/[\u00c0-\u00ff]\u0080?|\uFFFD/u.test(slicerPage),
  'SlicerPage has no visible UTF-8 mojibake in the Smart Cut commercial workbench source',
);
assertIncludes(slicerPage, 'Human Review', 'Slicer UI reserves an explicit human review checkpoint before publishable output');
assertIncludes(slicerPage, 'Review before render', 'Slicer UI exposes a real human-in-the-loop review mode before native rendering');
assertIncludes(slicerPage, 'Auto render', 'Slicer UI keeps one-click automatic slicing as a first-class mode');
assertIncludes(slicerPage, 'segmentationDensity', 'Slicer UI stores segmentation density as a real Smart Cut Engine option');
assertIncludes(slicerPage, 'Default segmentation', 'Slicer UI exposes the default segmentation density option');
assertIncludes(slicerPage, 'Maximize continuous content', 'Slicer UI exposes a maximize-continuity segmentation density option');
assertIncludes(slicerPage, 'Merge continuous semantic units up to max duration', 'Slicer UI explains that maximize mode merges continuous content within duration bounds');
...
assertIncludes(slicerPage, 'Speech-to-text mode', 'Slicer UI exposes a product-facing STT strategy selector for long-video slicing');
assertIncludes(slicerPage, 'selectedSttWorkflowPreset.localWhisper', 'Slicer UI surfaces the selected local Whisper chunk execution settings');
...
assertIncludes(slicerPage, 'requires GPU runtime', 'Slicer UI tells operators when GPU local STT requires a GPU-enabled whisper.cpp sidecar');
...
assertIncludes(slicerPage, 'Segment Review Workbench', 'Slicer UI provides a dedicated segment review workbench instead of a passive human-review note');
assertIncludes(slicerPage, 'Render selected', 'Slicer UI lets operators render only manually selected review segments');
assertIncludes(slicerPage, 'selectedReviewSegmentIds', 'Slicer UI tracks checkbox-based manual segment selection');
...
assertIncludes(slicerPage, 'handlePreviewReviewSegment', 'Slicer UI supports explicit preview seeking for review segments');
...
assertIncludes(slicerPage, 'Review Queue', 'Slicer UI presents human review as a queue instead of only a passive segment list');
assertIncludes(slicerPage, 'Previewing Segment', 'Slicer UI shows the currently previewed segment evidence before rendering');
assertIncludes(slicerPage, 'Select all publishable', 'Slicer UI has a one-click publishable segment selection action');
assertIncludes(slicerPage, 'Clear selection', 'Slicer UI has a one-click selection clearing action');
assertIncludes(slicerPage, 'All segments', 'Slicer UI has an all-segments review filter');
assertIncludes(slicerPage, 'Selected only', 'Slicer UI has a selected-segments review filter');
assertIncludes(slicerPage, 'Duplicates only', 'Slicer UI has a duplicate-content review filter');
assertIncludes(slicerPage, 'Excluded only', 'Slicer UI has an excluded-segments review filter');
assertIncludes(slicerPage, 'handleSplitReviewSegment', 'Slicer UI supports manual split at an engine-owned segment boundary');
...
assertIncludes(slicerPage, 'Commercial Readiness', 'Slicer UI gives operators one launch-readiness summary before running Smart Cut Engine');
assertIncludes(slicerPage, 'Source Evidence', 'Slicer UI surfaces source evidence readiness as a commercial gate');
assertIncludes(slicerPage, 'Speaker Evidence', 'Slicer UI surfaces speaker evidence readiness as a commercial gate');
assertIncludes(slicerPage, 'Export Contract', 'Slicer UI surfaces export contract readiness as a commercial gate');
assertIncludes(slicerPage, 'commercialReadinessItems', 'Slicer derives commercial readiness items from source, speaker, and export evidence');
assertIncludes(slicerPage, 'hasCommercialReadinessBlocker', 'Slicer derives a launch blocker from commercial readiness instead of ad-hoc button logic');
assertIncludes(slicerPage, 'disabled={isProcessing || hasCommercialReadinessBlocker}', 'Slicer start action is disabled by commercial readiness blockers');
...
assertNotIncludes(slicerPage, 'disabled={isProcessing || !hasVideoSource}', 'Slicer start action is not gated by a narrow source-only condition');
...
assertIncludes(slicerPage, 'Speech-to-text evidence', 'Slicer UI exposes speech-to-text evidence as the first Smart Cut Engine step');
assertIncludes(slicerPage, 'Speaker diarization', 'Slicer UI exposes speaker diarization as a first-class Smart Cut Engine step');
assertIncludes(slicerPage, 'Semantic content units', 'Slicer UI exposes semantic content units before any filter planning');
assertIncludes(slicerPage, 'Candidate ID review', 'Slicer UI presents LLM review as candidate-id ranking instead of timestamp generation');
assertIncludes(slicerPage, 'Post-filter render', 'Slicer UI presents denoise, silence, abnormal-fragment, and repeat removal as post-boundary filters');
assertIncludes(slicerPage, 'smartCutRequiresSpeakerDiarization', 'Slicer UI derives multi-speaker diarization requirements from the selected scene preset');
assertIncludes(slicerPage, 'Multi-speaker gate', 'Slicer UI visibly tells operators that interview/dialogue/meeting modes require speaker diarization');
...
assertIncludes(taskDetailReviewPage, 'Segment Review Workbench is ready', 'TaskDetailPage presents a dedicated review-ready state instead of a generic progress view');
assertIncludes(taskDetailReviewPage, 'Open review workbench', 'TaskDetailPage provides a clear action to continue manual segment review');
...
assertIncludes(taskDetailReviewPage, 'SMART_SLICE_EVIDENCE_PACKAGE_ITEMS', 'TaskDetailPage defines the canonical Smart Slice evidence package for task detail inspection');
assertIncludes(taskDetailReviewPage, 'SMART_SLICE_EVIDENCE_STEP_IDS', 'TaskDetailPage counts only evidence-producing checkpoint steps in the evidence inspector summary');
assertIncludes(taskDetailReviewPage, 'createSmartSliceEvidenceInspectorRows', 'TaskDetailPage derives Smart Slice evidence inspector rows from the workflow checkpoint');
assertIncludes(taskDetailReviewPage, 'Smart Slice Evidence Inspector', 'TaskDetailPage exposes a task-scoped Smart Slice Evidence Inspector');
assertIncludes(taskDetailReviewPage, 'copiedSmartSliceEvidenceItemId', 'TaskDetailPage scopes copied feedback to the selected Smart Slice evidence artifact');
assertIncludes(taskDetailReviewPage, 'copySmartSliceEvidenceArtifactPath', 'TaskDetailPage lets operators copy each task evidence artifact path');
assertIncludes(taskDetailReviewPage, 'openSmartSliceEvidenceArtifactLocation', 'TaskDetailPage lets operators reveal task evidence artifacts from the inspector');
assertIncludes(taskDetailReviewPage, "relativePath: 'evidence/speech-to-text.json'", 'TaskDetailPage inspector includes the canonical speech-to-text evidence file');
assertIncludes(taskDetailReviewPage, "relativePath: 'evidence/semantic-segmentation.json'", 'TaskDetailPage inspector includes the canonical semantic segmentation evidence file');
assertIncludes(taskDetailReviewPage, "relativePath: 'evidence/review-session.json'", 'TaskDetailPage inspector includes the canonical review session evidence file');
assertIncludes(taskDetailReviewPage, "relativePath: 'evidence/manual-edits.json'", 'TaskDetailPage inspector includes the canonical manual edits evidence file');
assertIncludes(taskDetailReviewPage, "relativePath: 'evidence/review-events.json'", 'TaskDetailPage inspector includes the canonical replayable review events evidence file');
assertIncludes(taskDetailReviewPage, "relativePath: 'evidence/render-selection.json'", 'TaskDetailPage inspector includes the canonical render selection evidence file');
assertIncludes(taskDetailReviewPage, "relativePath: 'evidence/render-artifact-manifest.json'", 'TaskDetailPage inspector includes the canonical render artifact manifest evidence file');
assertIncludes(slicerPage, 'contentUnitIds + speakerRoles', 'Slicer UI surfaces audit-ready content unit and speaker role evidence');
assertIncludes(slicerPage, 'Semantic boundaries first, filters second', 'Slicer UI communicates that filters run after accepted semantic boundaries');
...
assertIncludes(taskDetailPage, "translationText ? 'Copy translation' : 'Copy transcript'", 'TaskDetailPage copy action distinguishes target-language translation from source transcript evidence');
...
assertIncludes(
  ...
  'TaskDetailPage wraps task content in a scrollable min-height constrained region so failed diagnostics can be copied',
);
...
assertIncludes(taskDetailPage, 'const selectedSliceReviewIssueCodes = selectedSlice ? createSmartSliceReviewIssueCodes(selectedSlice) : []', 'TaskDetailPage computes canonical smart slice review issue codes once per selected slice render');
assertIncludes(taskDetailPage, 'selectedSliceReviewIssueCodes.map', 'TaskDetailPage displays all smart slice review issues through the canonical review risk renderer');
assertRule(!taskDetailPage.includes('createSmartSliceReviewIssueCodes(selectedSlice).map'), 'TaskDetailPage does not recompute canonical smart slice review issue codes during review card rendering');
assertIncludes(taskDetailPage, 'createSmartSliceReviewIssueCodes(slice).length', 'TaskDetailPage opens the smart slice review panel when canonical review issue codes exist even without other metadata');
...
assertIncludes(taskDetailPage, 'isSmartSliceReviewSentenceBoundaryIssue', 'TaskDetailPage filters clean/repaired sentence boundary states out of human-review warnings');
...
assertIncludes(taskDetailPage, 'const [showSlicingLogic, setShowSlicingLogic]', 'TaskDetailPage owns a click-to-view smart slicing logic disclosure state');
assertIncludes(taskDetailPage, 'taskDetail.slicingLogic.title', 'TaskDetailPage labels the task-level smart slicing logic view');
assertIncludes(taskDetailPage, 'taskDetail.slicingLogic.sliceCount', 'TaskDetailPage displays the smart slicing result count in the logic view');
...
assertIncludes(taskDetailPage, 'createSmartSliceTaskSlicingLogicSummary(sliceResults)', 'TaskDetailPage summarizes task-level smart slicing logic from the completed slice results');
...
assertIncludes(taskDetailPage, 'slice.reason', 'TaskDetailPage explains why every smart slice was selected in the slicing logic view');
assertIncludes(taskDetailPage, 'formatSmartSliceBoundaryDecisionSource(slice.boundaryDecisionSource)', 'TaskDetailPage shows whether each smart slice boundary came from transcript, audio, or combined analysis');
assertIncludes(taskDetailPage, 'formatSliceContentArcGrade(slice.contentArcGrade)', 'TaskDetailPage shows per-slice content arc grades in the slicing logic view');
assertIncludes(taskDetailPage, 'formatSliceTopicCoherenceGrade(slice.topicCoherenceGrade)', 'TaskDetailPage shows per-slice topic coherence grades in the slicing logic view');
assertIncludes(taskDetailPage, 'slice.transcriptSegmentCount', 'TaskDetailPage shows per-slice speech transcript evidence counts in the slicing logic view');
assertIncludes(taskDetailPage, 'AUTOCUT_SMART_SLICE_REVIEW_RISK_CATALOG', 'TaskDetailPage displays smart slice review risks through the canonical risk catalog');
assertIncludes(taskDetailPage, 'getSmartSliceReviewRiskDefinition', 'TaskDetailPage resolves human-readable review risk labels instead of exposing raw risk codes only');
assertIncludes(taskDetailPage, 'taskDetail.reviewRisk.title', 'TaskDetailPage labels smart slice review warnings with i18next copy');
...
assertIncludes(slicerPlannerCheck, 'audio cleanup boundary conflicts choose renderable audio activity boundaries instead of transcript windows with excessive trusted padding', 'Slicer planner check covers renderable audio boundaries when trusted audio activity conflicts with transcript timing');
...
assertIncludes(taskDetailPage, 'handleCopyTranscriptText', 'TaskDetailPage lets operators click recognized transcript text to copy correction evidence');
assertIncludes(taskDetailPage, "t('taskDetail.transcript.copySegmentAria'", 'TaskDetailPage exposes localized per-segment transcript copy actions');
assertIncludes(taskDetailPage, 'handleStartTranscriptEdit', 'TaskDetailPage lets operators start correcting recognized slice transcripts');
assertIncludes(taskDetailPage, 'handleSaveTranscriptEdit', 'TaskDetailPage lets operators save corrected recognized slice transcripts');
assertIncludes(taskDetailPage, 'updateTaskSliceTranscript', 'TaskDetailPage persists corrected slice transcript text through the task service');
assertIncludes(taskDetailPage, 'selectedSlice.transcriptCorrection', 'TaskDetailPage displays manual transcript correction audit status for selected smart slices');
...
assertIncludes(taskDetailPage, "t('taskDetail.transcript.copyAll')", 'TaskDetailPage localizes the transcript copy-all action');
assertIncludes(taskDetailPage, "t('taskDetail.transcript.segmentCopied')", 'TaskDetailPage localizes per-segment transcript copy feedback');
assertIncludes(taskDetailPage, "t('taskDetail.transcript.save')", 'TaskDetailPage localizes the transcript save action');
assertIncludes(taskDetailPage, "t('taskDetail.transcript.edit')", 'TaskDetailPage localizes the transcript edit action');
assertIncludes(taskDetailPage, "t('taskDetail.transcript.copyFailed')", 'TaskDetailPage localizes transcript copy failures');
...
assertIncludes(taskDetailPage, "t('taskDetail.transcript.exportTxt')", 'TaskDetailPage labels the slice transcript export separately from optional subtitle downloads');
...
assertIncludes(slicerPage, 'selectedSegmentationAgent.systemPrompt', 'Slicer shows the selected segmentation agent system prompt before execution');
...
assertIncludes(slicerPage, 'Strategy Capability', 'SlicerPage shows a commercial readiness gate for strategy execution capability');
assertIncludes(slicerPage, 'Native evidence adapter required', 'SlicerPage blocks unimplemented visual/audio native strategies before task submission');
assertIncludes(slicerPage, 'data-smart-cut-strategy-status', 'SlicerPage mode cards expose per-strategy execution status for commercial users');
assertIncludes(slicerPage, "modeProfile.executionSupport.ready ? 'Ready' : 'Blocked'", 'SlicerPage mode cards label runnable and blocked Smart Cut Engine strategies explicitly');
assertIncludes(slicerPage, 'modeProfile.executionSupport.detail', 'SlicerPage mode cards surface the exact capability detail before users submit a job');
